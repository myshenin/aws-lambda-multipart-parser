...
module.exports.parse = (event, spotText) => {
    ...
    const result = {};
    const contentTypeRegex = /Content-Type:\s.+/gi;
    event.body
        .split(boundary)
        .forEach(item => {
            if (/filename=".+"/g.test(item)) {
                result[item.match(/name=".+";/g)[0].slice(6, -2)] = {
                    ...
                    contentType: item.match()[0].slice(14),
                    content: spotText? Buffer.from(item.slice(item.search(contentTypeRegex) + item.match(contentTypeRegex)[0].length + 4, -4), 'binary'):
                        item.slice(item.search(contentTypeRegex) + item.match(contentTypeRegex)[0].length + 4, -4),
                };
